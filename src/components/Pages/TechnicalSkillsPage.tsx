...
export default function TechnicalSkillsPage() {
  ...
  const categories = [
    { id: "all", name: `${t("technical.skills.categories.all")}`, icon: <Layers className="w-5 h-5" /> },
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          {t("technical.skills.sectionTitle")}
          </h1>
          <p className="text-lg opacity-90 max-w-2xl">
          {t("technical.skills.sectionSubtitle")}
          </p>
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-8">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder={`${t("technical.skills.searchPlaceholder")}`}
      ...
      <div className="mb-16">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">
            ...
            {t("technical.skills.languagesAndLibraries.title")}
            </span>
          </h2>
          <div className="text-gray-500 flex items-center">
            <Filter className="w-4 h-4 mr-2" />
            <span>
            {t("technical.skills.languagesAndLibraries.show")} {filteredSkills.length} {t("technical.skills.languagesAndLibraries.of")} {skills.length} {t("technical.skills.languagesAndLibraries.skill")}
            </span>
          </div>
        </div>
          ...
          {filteredSkills.map((skill, index) => (
            <motion.div
              ...
              className={`bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all border border-gray-100 ${
                hoveredSkill === skill.name ? "ring-2 ring-blue-400" : ""
              }`}
            >
              <div
                className="h-2 w-full"
                style={{ backgroundColor: `${getColorByLevel(skill.level)}` }}
              ></div>
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    ...
                    {t("technical.skills.languagesAndLibraries.proficiency")}
                    </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                  <span className="text-sm font-medium text-gray-500">
                    <span className="text-blue-600 font-bold me-1">
                      {skill.years}
                    </span>
                    {t("technical.skills.languagesAndLibraries.experience")}
                  </span>
        ...
        {filteredSkills.length === 0 && (
          <div className="text-center py-16 bg-gray-50 rounded-xl">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">
            {t("technical.skills.languagesAndLibraries.noResult.title")}
            </h3>
            <p className="text-gray-600 mb-4">
            {t("technical.skills.languagesAndLibraries.noResult.description")}
            </p>
            <button
              onClick={() => {
                setSearchTerm("");
            ...
            >
              {t("technical.skills.languagesAndLibraries.noResult.button")}
            </button>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-500">
            {t("technical.skills.tools.title")}
            </span>
          </h2>
          {searchTerm && (
            <div className="text-gray-500 flex items-center">
              <Filter className="w-4 h-4 mr-2" />
              <span>
              {t("technical.skills.tools.show")} {filteredTools.length} {t("technical.skills.tools.of")} {tools.length} {t("technical.skills.tools.tool")}
              </span>
            </div>
          )}
        </div>
            ...
            >
              ...
              <div className="w-2/3 p-6">
                ...
                <div className="flex justify-between mt-2 text-xs text-gray-500">
                  <span>{t("technical.skills.tools.beginner")}</span>
                  <span>{t("technical.skills.tools.expert")}</span>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        {searchTerm && filteredTools.length === 0 && (
          <div className="text-center py-16 bg-gray-50 rounded-xl">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">
            {t("technical.skills.tools.noResult.title")}
            </h3>
            <p className="text-gray-600 mb-4">
            {t("technical.skills.tools.noResult.description")}
            </p>
            <button
              onClick={() => setSearchTerm("")}
              className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors"
            >
              {t("technical.skills.tools.noResult.button")}
            </button>
          </div>
        )}
      </div>
